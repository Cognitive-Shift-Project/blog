export interface OgLogoSvgOptions {
	logoHeightPx?: number
	/** CSS color value applied to the fill (e.g. `'white'` for dark OG backgrounds). */
	color?: string
}
...
// Used with fill-rule="evenodd" so the lobes create cutouts from the filled ring.
...
export function ogLogoSvg(options: OgLogoSvgOptions = {}): string {
	...
	return `\
	<svg xmlns="http://www.w3.org/2000/svg" width="${widthPx}" height="${logoHeightPx}" viewBox="0 0 64 64">
		<path fill="${color}" fill-rule="evenodd" d="${LOGO_PATH_D}"/>
	</svg>`
}
